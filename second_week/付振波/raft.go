...
type Raft struct {
	...
	commitIndex int //index of highest log entry known to be committes
...
type RequestVoteArgs struct {
	...
	CandidateId int
	LastLogIndex int //of candidated
	LastLogTerm int //of candidated
}
...
func (rf *Raft) Election() {
	...
	if rf.state != "LEADER" {
		...
		for i := 0; i < len(rf.peers); i++ {
			if i != rf.me {
				go func(server int, args RequestVoteArgs) {//xiecheng
...
func (rf *Raft) VoteResult(reply RequestVoteReply) {
	rf.mu.Lock()
	defer rf.mu.Unlock()
	...
	if reply.VoteGranted && rf.state == "CANDIDATE" {
		...
		if rf.voteCount > len(rf.peers)/2 {
			...
			rf.resetTimer()
			go func() { rf.RejectElectionAndSendLog() }()
		}
	}
}
...
type AppendEntriesArgs struct {
	...
	Entries  []LogEntry //logs entries to store
...
type AppendEntriesReply struct {
	...
	CommitIndex int //pipei leader.Term for tongbu log
}
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if rf.currentTerm > args.Term {
		...
		if args.PrevLogIndex >= 0 &&
			(len(rf.logs)-1 < args.PrevLogIndex || 
...
//leader reject election via sending AppendEntries
func (rf *Raft) RejectElectionAndSendLog() {
	for i := 0; i < len(rf.peers); i++ {
		if i == rf.me {
			continue
		}
		...
		go func(servernumber int, args AppendEntriesArgs, rf *Raft) {
			...
			if ok {
				rf.AppendEntriesResult(servernumber, reply)
			}else {
				goto retry
			}
		}(i, args, rf)
...
func (rf *Raft) AppendEntriesResult(server int, reply AppendEntriesReply) {
	rf.mu.Lock()
	defer rf.mu.Unlock()
	...
	if reply.Success {
		...
		if commit_count > len(rf.peers)/2 &&
...
func (rf *Raft) commitLogs() {
	...
	if rf.commitIndex > len(rf.logs)-1 {
		log.Fatal("error : raft.go commitlogs()")
	}
