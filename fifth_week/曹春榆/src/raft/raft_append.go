				...
				if ok {
					...
					if reply.Success && len(entries) != 0 {
						...
						if rf.commitId != oldCommitId {
							//if oldCommitId < 100 {
							//	fmt.Println("机器", rf.me, "新的CommitId为", rf.commitId, "lastApplied", rf.lastApplied,
							//		"commit的命令为", rf.logs.GetLog(rf.commitId))
							//}
							sleepTime = 0
						}
...
func (rf *Raft) AppendLogEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if rf.IsPrint {
		fmt.Println("\n\n机器", rf.me, "GID", rf.GID, "的commitId为", rf.commitId, "lastInclude", rf.logs.lastIncludeTerm, rf.logs.lastIncludeIndex,
			"snap数量为", rf.logs.snapNum, "log长度为", rf.logs.Size(), "内容为", rf.logs.log)
		fmt.Println("机器", rf.me, "GID", rf.GID, "在任期", rf.currentTerm, "收到leader", args.LeaderId, "发送的", args.Entries)
...
func (rf *Raft) applyLogLoop() {
	for !rf.killed() {
		...
		for rf.commitId > rf.lastApplied && rf.lastApplied < rf.logs.Size() {
			...
			//if rf.commitId < 100 && rf.isLeader && rf.lastApplied >= 7 {
			//	fmt.Println(rf.me, "rf.commitId, rf.lastApplied", rf.commitId, rf.lastApplied, rf.logs.Size())
			//}
			rf.lastApplied += 1
		...
		for _, msg := range appliedMsgs {
			...
			//if rf.commitId < 100 && rf.isLeader {
			//	fmt.Println(rf.me, "提交了消息", msg, rf.lastApplied)
			//}
			rf.applyCh <- msg
			//if rf.commitId < 100 && rf.isLeader {
			//	fmt.Println(rf.me, "提交完成", msg, rf.lastApplied)
			//}
		}
	}
}
