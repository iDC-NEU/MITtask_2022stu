...
func TestStaticShards(t *testing.T) {
	...
	DPrintln("before join 0")
	cfg.join(0)
	DPrintln("after join 0")
	cfg.join(1)
	DPrintln("after join 1")
	...
	for i := 0; i < n; i++ {
		...
		va[i] = randstring(20)
		DPrintln("before put ", i)
		ck.Put(ka[i], va[i])
		DPrintln("after put ", i)
	}
	for i := 0; i < n; i++ {
...
func TestMissChange(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		DPrintln("开始检查", i, ka[i], va[i])
		check(t, ck, ka[i], va[i])
		x := randstring(20)
		DPrintln("插入", i, ka[i], x)
		ck.Append(ka[i], x)
		DPrintln("插入成功", i, ka[i], x)
		va[i] += x
	}
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		x := randstring(20)
		DPrintln("插入二", i, ka[i], x)
		ck.Append(ka[i], x)
		DPrintln("插入二完成", i, ka[i], x)
		va[i] += x
	}
